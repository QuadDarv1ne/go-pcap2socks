...
try {
    ...
    $wifiInterface = Get-NetAdapter | Where-Object {
        $_.Status -eq 'Up' -and (
            ...
            $_.InterfaceDescription -match 'Wi-Fi|Wireless|802\.11|Centrino|AX200|AX201|AX210' -or
    ...
    $ethernetInterfaces = Get-NetAdapter | Where-Object {
        $_.Status -eq 'Up' -and (
            ...
            ($_.InterfaceDescription -match 'Ethernet|LAN|GbE|FastEther|Realtek|Intel.*I211|I350' -and
             $_.InterfaceType -ne 71 -and
             $_.NdisPhysicalMediumType -ne 9)  # Исключаем Wi-Fi
        )
    }
    ...
    if ($wifiInterface) {
        ...
        Write-Host "[OK] Wi-Fi ($($wifiInterface.InterfaceAlias)) приоритет: 25 (высокий)" -ForegroundColor Green
        ...
        foreach ($eth in $ethernetInterfaces) {
            ...
            Write-Host "[OK] Ethernet ($($eth.InterfaceAlias)) приоритет: 35 (средний)" -ForegroundColor Green
        }
    } else {
        Write-Host "INFO: Wi-Fi не найден, пропускаем настройку приоритета" -ForegroundColor Yellow
    }
} catch {
    Write-Host "WARNING: Не удалось настроить приоритет интерфейсов: $_" -ForegroundColor Yellow
}
Write-Host ""
